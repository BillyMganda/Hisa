...
namespace Hisa
{
    public partial class Book_Over : Form
    {
        public Book_Over()
        {
            InitializeComponent();           
        ...
        //LOAD
        private void Book_Over_Load(object sender, EventArgs e)
        {
        ...
        {
            if (txtReason.Text == "")
                MessageBox.Show("Reason must be included in a book over");
            else
            {
                try
                {
                    for (int i = 0; i < dataGridCheckOut.Rows.Count; i++)
                        using (SqlConnection sqlcon = new SqlConnection(connectionstring))
                        {
                            if (sqlcon.State == ConnectionState.Closed)
                                sqlcon.Open();
                            SqlCommand sqlcmd = new SqlCommand("Book_Over_Add", sqlcon);
                    ...
                    MessageBox.Show("Book Over Succcessful");
        ...
        {
            Print_Book_Over bo=new Print_Book_Over();
            bo.ShowDialog();
        }
    }
}
