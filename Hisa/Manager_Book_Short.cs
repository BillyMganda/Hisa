...
namespace Hisa
{
    public partial class Manager_Book_Short : Form
    {
        public Manager_Book_Short()
        {
            InitializeComponent();
        }
        ...
        {
            string querry = "SELECT * FROM Book_Short WHERE Record_Number=(SELECT MAX(Record_Number) FROM Book_Short)";
        ...
        void FillDatagrid()
        {
            try
            {
                ...
                {
                    sqlcon.Open();
                    SqlDataAdapter sqlda = new SqlDataAdapter($"SELECT ItemName,ItemCode,Units,Book_Value,UnitPrice,Total_Book_Value FROM Book_Short WHERE Record_Number='" + Convert.ToInt32(txtRecordNO.Text) + "'", sqlcon);
        ...
        //LOAD
        private void Manager_Book_Short_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection sqlcon = new SqlConnection(connectionstring))
                {
                    if (sqlcon.State == ConnectionState.Closed)
                        sqlcon.Open();
                    string querry = $"DELETE FROM Book_Short WHERE Record_Number='" + Convert.ToInt32(txtRecordNO.Text) + "'";
        ...
        void RefreshWindow()
        {
            Manager_Book_Short mr = new Manager_Book_Short();
            mr.ShowDialog();
        }
                ...
                try
                {
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        using (SqlConnection sqlcon = new SqlConnection(connectionstring))
                        {
                            if (sqlcon.State == ConnectionState.Closed)
                                sqlcon.Open();
                            SqlCommand sqlcmd = new SqlCommand("Void_Book_Short_Add", sqlcon);
                ...
                try
                {
                    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        using (SqlConnection sqlcon = new SqlConnection(connectionstring))
                        {
                            if (sqlcon.State == ConnectionState.Closed)
                                sqlcon.Open();
                            SqlCommand sqlcmd = new SqlCommand("Authorize_Book_Short_Add", sqlcon);
                            sqlcmd.CommandType = CommandType.StoredProcedure;
                            sqlcmd.Parameters.AddWithValue("@Date", DateTime.Parse(txtDate.Text));
                            sqlcmd.Parameters.AddWithValue("@time", DateTime.Parse(txtTime.Text));
                            sqlcmd.Parameters.AddWithValue("@Record_Number", Convert.ToInt32(txtRecordNO.Text));
