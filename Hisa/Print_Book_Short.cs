...
namespace Hisa
{
    public partial class Print_Book_Short : Form
    {
        public Print_Book_Short()
        {
            InitializeComponent();
        }
        ...
        public void Load_Details()
        {
            try
            {
                ...
                string querry = "SELECT * FROM Authorize_Book_Short WHERE Record_Number=(SELECT MAX(Record_Number) FROM Authorize_Book_Short)";
        ...
        private void FillDataGrid()
        {
            try
            {
                ...
                {
                    sqlcon.Open();
                    SqlDataAdapter sqlda = new SqlDataAdapter($"SELECT ItemName,ItemCode,Units,UnitPrice,Book_Value,Total_Book_Value FROM Authorize_Book_Short WHERE Record_Number='" + Convert.ToInt32(textBox5.Text) + "'", sqlcon);
            ...
            {
                MessageBox.Show("No new book short to print at this time");
            }
        }
        //LOAD
        private void Print_Book_Short_Load(object sender, EventArgs e)
        {
            Load_Details();
            FillDataGrid();
        ...
        public void Refresh()
        {
            Print_Book_Short pprint = new Print_Book_Short();
            pprint.ShowDialog();
        }
        ...
        private void btnAddToCart_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    SqlConnection con = new SqlConnection(connectionstring);
                    con.Open();
                    string querry = "INSERT INTO Book_Short_Done(Date,time,Record_By,Authorised_By,Record_Number,ItemName,ItemCode,Units,UnitPrice,Book_Value,Total_Book_Value,Book_Reason)SELECT Date,time,Record_By,Authorised_By,Record_Number,ItemName,ItemCode,Units,UnitPrice,Book_Value,Total_Book_Value,Book_Reason FROM Authorize_Book_Short WHERE Record_Number='" + Convert.ToInt32(textBox5.Text) + "' DELETE FROM Authorize_Book_Short WHERE Record_Number='" + Convert.ToInt32(textBox5.Text) + "'";
                ...
                //PRINT
                To_BookShort book = new To_BookShort();
                book.ShowDialog();
            }
        }
    }
